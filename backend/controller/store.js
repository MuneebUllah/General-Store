...
const addDataById = async (req, res) => {
    try {
        const { name, category, quantity, price, noOfItems, size, weight } = req.body;
        ...
        return res.json({ message: 'Quantity updated successfully', data: updatedItem });
    } catch (error) {
...
const removeDataById = async (req, res) => {
    try {
        ...
        if (item.quantity <= 0) {
            ...
            return res.json({ message: 'Item removed successfully as quantity reached zero' });
        } else {
            const updatedItem = await item.save();
