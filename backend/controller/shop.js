...
const getAllCategories = async (req, res) => {
  try {
    ...
    const categories = await shopModal.aggregate([
      {
        $group: {
          _id: "$category",
...
const getAllNames = async (req, res) => {
  try {
    ...
    const names = await shopModal.aggregate([
      {
        $group: {
          _id: "$name",
          count: { $sum: 1 },
...
const getAllPrices = async (req, res) => {
  try {
    ...
    const prices = await shopModal.distinct("price");
...
const getAllSizes = async (req, res) => {
  try {
    ...
    const sizes = await shopModal.distinct("size");
...
const getDataById = async (req, res) => {
  try {
    const data = await shopModal.findById(req.params.id);
    if (!data) {
...
const getDataByCategory = async (req, res) => {
  try {
    ...
    const data = await shopModal.aggregate([
...
const getDataByName = async (req, res) => {
  try {
    ...
    // Using aggregation to group by 'name' and calculate totalAmount and count
    const data = await shopModal.find({ name });
...
const createData = async (req, res) => {
  ...
  if (!name || !category || !salePrice || !purchasePrice || !quantity) {
...
const addDataById = async (req, res) => {
  try {
    ...
    const item = await shopModal.findOne(query);
...
const deleteDataById = async (req, res) => {
  ...
  const dataById = await shopModal.findByIdAndDelete(req.params.id);
...
//             return res.status(404).json({ message: 'No history data found for this name' });
//         }
...
const searchByCategory = async (req, res) => {
  try {
    ...
    const categoryData = await shopModal.aggregate([
      {
        $match: {
          category: {
...
const searchByName = async (req, res) => {
  try {
    ...
    // Aggregate data for the specific category
    const nameData = await shopModal.aggregate([
      {
        $match: {
          name: {
            $regex: new RegExp(name, "i"), // Case-insensitive partial match
